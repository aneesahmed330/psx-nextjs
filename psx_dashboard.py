...
def add_alert(symbol, min_price, max_price, enabled):
    ...
    db.alerts.insert_one({
        ...
        'trigger': False  # Default value is False
    })
...
def set_alert_trigger(symbol, min_price, max_price, trigger):
    db = get_mongo()
    db.alerts.update_one(
        {'symbol': symbol, 'min_price': float(min_price), 'max_price': float(max_price)},
    ...
    with tab2:
        ...
        if not portfolio_df.empty:
            ...
            if price_history:
                ...
                # --- Min/Max Analysis Table ---
                st.markdown("#### 📊 Price Range Analysis")
                st.markdown("Shows the minimum and maximum normalized prices reached by each symbol, along with actual prices at those points.")
                
                # Calculate min/max for each symbol
                analysis_data = []
                for symbol in norm_symbols:
                    sym_df = norm_price_df[norm_price_df['symbol'] == symbol].copy()
                    if not sym_df.empty:
                        # Find min and max normalized prices
                        min_idx = sym_df['norm_price'].idxmin()
                        max_idx = sym_df['norm_price'].idxmax()
                        
                        min_norm = sym_df.loc[min_idx, 'norm_price']
                        max_norm = sym_df.loc[max_idx, 'norm_price']
                        min_price = sym_df.loc[min_idx, 'price']
                        max_price = sym_df.loc[max_idx, 'price']
                        min_time = sym_df.loc[min_idx, 'fetched_at_pkt']
                        max_time = sym_df.loc[max_idx, 'fetched_at_pkt']
                        
                        # Calculate percentage changes
                        min_change = (min_norm - 1.0) * 100
                        max_change = (max_norm - 1.0) * 100
                        range_spread = (max_norm - min_norm) * 100
                        
                        analysis_data.append({
                            'Symbol': symbol,
                            'Min Normalized': f"{min_norm:.3f}",
                            'Min Price': f"Rs. {min_price:.2f}",
                            'Min Change': f"{min_change:+.1f}%",
                            'Min Time': min_time.strftime('%b %d %H:%M'),
                            'Max Normalized': f"{max_norm:.3f}",
                            'Max Price': f"Rs. {max_price:.2f}",
                            'Max Change': f"{max_change:+.1f}%",
                            'Max Time': max_time.strftime('%b %d %H:%M'),
                            'Range Spread': f"{range_spread:.1f}%"
                        })
                
                if analysis_data:
                    analysis_df = pd.DataFrame(analysis_data)
                    
                    # Color coding function for the changes
                    ...
                    # Apply styling
                    styled_analysis = analysis_df.style.applymap(
                        color_change, 
                        subset=['Min Change', 'Max Change']
                    )
                    ...
                    st.dataframe(
                        ...
                        column_config={
                            ...
                            "Min Normalized": st.column_config.TextColumn("Min Norm.", width="small"),
                            "Min Price": st.column_config.TextColumn("Min Price", width="medium"),
                            "Min Change": st.column_config.TextColumn("Min %", width="small"),
                            "Min Time": st.column_config.TextColumn("Min Time", width="medium"),
                            "Max Normalized": st.column_config.TextColumn("Max Norm.", width="small"),
                            "Max Price": st.column_config.TextColumn("Max Price", width="medium"),
                            "Max Change": st.column_config.TextColumn("Max %", width="small"),
                            "Max Time": st.column_config.TextColumn("Max Time", width="medium"),
                            "Range Spread": st.column_config.TextColumn("Range", width="small")
                        }
                    )
                    ...
                    min_changes = [float(d['Min Change'].replace('%', '').replace('+', '')) for d in analysis_data]
                    max_changes = [float(d['Max Change'].replace('%', '').replace('+', '')) for d in analysis_data]
                    range_spreads = [float(d['Range Spread'].replace('%', '')) for d in analysis_data]
                    
                    with col1:
                        best_performer = analysis_data[max_changes.index(max(max_changes))]
                        st.info(f"🚀 **Best Peak**: {best_performer['Symbol']} reached {best_performer['Max Change']} at {best_performer['Max Time']}")
                    
                    with col2:
                        worst_performer = analysis_data[min_changes.index(min(min_changes))]
                        st.warning(f"📉 **Lowest Dip**: {worst_performer['Symbol']} dropped to {worst_performer['Min Change']} at {worst_performer['Min Time']}")
                    
                    with col3:
                        most_volatile = analysis_data[range_spreads.index(max(range_spreads))]
                        st.error(f"⚡ **Most Volatile**: {most_volatile['Symbol']} with {most_volatile['Range Spread']} range")
                    
                    st.caption("*All percentages are relative to the starting price (1.0 = 0% change)*")
                else:
    ...
    with tab4:
        ...
        if not alerts_df.empty:
            ...
            for idx, alert in alerts_df.iterrows():
                ...
                # Trigger status
                trigger_status = alert.get('trigger', False)  # Default to False if not set
                trigger_icon = "🔥" if trigger_status else "⭕"
                trigger_label = "Triggered" if trigger_status else "Waiting"
                trigger_color = "#FF4560" if trigger_status else "#A0AEC0"
                row_cols[4].markdown(
                    f"<span style='color:{trigger_color};font-weight:bold;'>{trigger_icon} {trigger_label}</span>",
                    unsafe_allow_html=True
                )
                ...
                with row_cols[5]:
                    ...
                    trigger_reset_label = "Reset" if trigger_status else "Set"
                    if tcol2.button(trigger_reset_label, key=f"trigger_{idx}"):
                        set_alert_trigger(alert['symbol'], alert['min_price'], alert['max_price'], not trigger_status)
                        st.success(f"Trigger {'reset' if trigger_status else 'set'} for {alert['symbol']}")
                        st.rerun()
                    
                    if tcol3.button("Delete", key=f"delete_{idx}"):
